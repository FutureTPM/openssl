  ...
  if (req) {
    ...
    if (spkac_file != NULL) {
      ...
      j = certify_spkac(&x, spkac_file, pkey, x509, dgst, sigopts, attribs, db,
                        ...
                        get_nameopt(), default_op, ext_copy);
      if (j < 0)
        goto end;
      if (j > 0) {
        total_done++;
    ...
    if (ss_cert_file != NULL) {
      total++;
      j = certify_cert(&x, ss_cert_file, pkey, x509, dgst, sigopts, attribs, db,
                       serial, subj, chtype, multirdn, email_dn, startdate,
    ...
    if (infile != NULL) {
      ...
      j = certify(&x, infile, pkey, x509p, dgst, sigopts, attribs, db, serial,
                  ...
                  batch, extensions, conf, verbose, certopt, get_nameopt(),
    ...
    for (i = 0; i < argc; i++) {
      total++;
      j = certify(&x, argv[i], pkey, x509p, dgst, sigopts, attribs, db, serial,
                  subj, chtype, multirdn, email_dn, startdate, enddate, days,
                  batch, extensions, conf, verbose, certopt, get_nameopt(),
                  default_op, ext_copy, selfsign);
      if (j < 0)
...
static int certify_cert(X509 **xret, const char *infile, EVP_PKEY *pkey,
  ...
  ok =
      do_body(xret, pkey, x509, dgst, sigopts, policy, db, serial, subj, chtype,
              ...
              rreq, ext_sect, lconf, certopt, nameopt, default_op, ext_copy, 0);
...
static int certify_spkac(X509 **xret, const char *infile, EVP_PKEY *pkey,
  ...
  EVP_PKEY_free(pktmp);
  ok = do_body(xret, pkey, x509, dgst, sigopts, policy, db, serial, subj,
               chtype, multirdn, email_dn, startdate, enddate, days, 1, verbose,
               req, ext_sect, lconf, certopt, nameopt, default_op, ext_copy, 0);
end:
  X509_REQ_free(req);
  CONF_free(parms);
