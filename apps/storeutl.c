...
int storeutl_main(int argc, char *argv[]) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    ...
    case OPT_CRITERION_ISSUER:
      if (criterion != 0 ||
          (criterion == OSSL_STORE_SEARCH_BY_ISSUER_SERIAL && issuer != NULL)) {
        BIO_printf(bio_err, "%s: criterion already given.\n", prog);
        goto end;
      }
    ...
    case OPT_CRITERION_SERIAL:
      if (criterion != 0 ||
          (criterion == OSSL_STORE_SEARCH_BY_ISSUER_SERIAL && serial != NULL)) {
        BIO_printf(bio_err, "%s: criterion already given.\n", prog);
        goto end;
      }
      criterion = OSSL_STORE_SEARCH_BY_ISSUER_SERIAL;
  ...
  for (;;) {
    ...
    case OSSL_STORE_INFO_NAME:
      if (recursive) {
        ...
        ret += process(suburi, uimeth, uidata, expected, criterion, search,
