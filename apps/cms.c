...
int cms_main(int argc, char **argv) {
    ...
    case OPT_RECIP:
      if (operation == SMIME_ENCRYPT) {
        if (encerts == NULL && (encerts = sk_X509_new_null()) == NULL)
          goto end;
        cert = load_cert(opt_arg(), FORMAT_PEM, "recipient certificate file");
        if (cert == NULL)
          goto end;
  ...
  if (operation == SMIME_ENCRYPT) {
    ...
    if (*argv && encerts == NULL)
      if ((encerts = sk_X509_new_null()) == NULL)
        goto end;
    while (*argv) {
      if ((cert = load_cert(*argv, FORMAT_PEM, "recipient certificate file")) ==
          NULL)
        goto end;
      sk_X509_push(encerts, cert);
      cert = NULL;
  ...
  if (operation == SMIME_DATA_CREATE) {
    ...
    cms =
        CMS_EncryptedData_encrypt(in, cipher, secret_key, secret_keylen, flags);
