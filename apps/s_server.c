...
static int ssl_servername_cb(SSL *s, int *ad, void *arg) {
  ...
  if (servername != NULL && p->biodebug != NULL) {
    ...
    while ((uc = *cp++) != 0)
      BIO_printf(p->biodebug, isascii(uc) && isprint(uc) ? "%c" : "\\x%02x",
                 uc);
...
static int get_ocsp_resp_from_responder(SSL *s, tlsextstatusctx *srctx,
  ...
  if (aia != NULL) {
    if (!OCSP_parse_url(sk_OPENSSL_STRING_value(aia, 0), &host, &port, &path,
                        &use_ssl)) {
...
int s_server_main(int argc, char *argv[]) {
  ...
  if (early_data && (www > 0 || rev)) {
    BIO_printf(bio_err, "Can't use -early_data in combination with -www, -WWW, "
                        "-HTTP, or -rev\n");
    goto end;
  }
  ...
  if (bio_s_out == NULL) {
    ...
    !defined(OPENSSL_NO_EC) || !defined(OPENSSL_NO_DILITHIUM)
  if (nocert)
#endif
  {
    s_cert_file = NULL;
