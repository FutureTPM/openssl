...
#else
static double Time_F(int s) { return app_tminterval(s, usertime); }
#endif
...
static int EVP_Digest_MD2_loop(void *args) {
  ...
  for (count = 0; COND(c[D_MD2][testnum]); count++) {
    if (!EVP_Digest(buf, (size_t)lengths[testnum], md2, NULL, EVP_md2(), NULL))
      return -1;
  }
  return count;
}
...
static int EVP_Digest_MD4_loop(void *args) {
  ...
  for (count = 0; COND(c[D_MD4][testnum]); count++) {
    if (!EVP_Digest(buf, (size_t)lengths[testnum], md4, NULL, EVP_md4(), NULL))
      return -1;
  }
  return count;
}
...
static int DES_ncbc_encrypt_loop(void *args) {
  ...
  for (count = 0; COND(c[D_CBC_DES][testnum]); count++)
    DES_ncbc_encrypt(buf, buf, lengths[testnum], &sch, &DES_iv, DES_ENCRYPT);
  return count;
}
...
static int EVP_Update_loop_aead(void *args) {
  ...
  if (decrypt) {
    for (count = 0; COND(nb_iter); count++) {
      EVP_DecryptInit_ex(ctx, NULL, NULL, NULL, iv);
      EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, sizeof(faketag), faketag);
...
#error "You cannot disable DES on systems without SIGALRM."
  ...
  if (doit[D_GHASH]) {
    for (i = 0; i < loopargs_len; i++) {
      ...
      CRYPTO_gcm128_setiv(loopargs[i].gcm_ctx, (unsigned char *)"0123456789ab",
                          12);
    }
    ...
    for (testnum = 0; testnum < size_num && async_init == 0; testnum++) {
      ...
      for (count = 0, run = 1; COND(c[D_CBC_128_CML][testnum]); count++)
        Camellia_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                             (size_t)lengths[testnum], &camellia_ks1, iv,
                             CAMELLIA_ENCRYPT);
      d = Time_F(STOP);
  ...
  if (doit[D_CBC_192_CML]) {
    ...
    for (testnum = 0; testnum < size_num && async_init == 0; testnum++) {
      ...
      for (count = 0, run = 1; COND(c[D_CBC_192_CML][testnum]); count++)
        Camellia_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                             (size_t)lengths[testnum], &camellia_ks2, iv,
                             CAMELLIA_ENCRYPT);
      d = Time_F(STOP);
  ...
  if (doit[D_CBC_256_CML]) {
    ...
    for (testnum = 0; testnum < size_num && async_init == 0; testnum++) {
      ...
      for (count = 0, run = 1; COND(c[D_CBC_256_CML][testnum]); count++)
        Camellia_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                             (size_t)lengths[testnum], &camellia_ks3, iv,
                             CAMELLIA_ENCRYPT);
      d = Time_F(STOP);
    ...
    for (testnum = 0; testnum < size_num && async_init == 0; testnum++) {
      ...
      for (count = 0, run = 1; COND(c[D_CBC_IDEA][testnum]); count++)
        IDEA_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                         (size_t)lengths[testnum], &idea_ks, iv, IDEA_ENCRYPT);
      d = Time_F(STOP);
    ...
    for (testnum = 0; testnum < size_num && async_init == 0; testnum++) {
      ...
      for (count = 0, run = 1; COND(c[D_CBC_RC2][testnum]); count++)
        RC2_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                        (size_t)lengths[testnum], &rc2_ks, iv, RC2_ENCRYPT);
      d = Time_F(STOP);
    ...
    for (testnum = 0; testnum < size_num && async_init == 0; testnum++) {
      ...
      for (count = 0, run = 1; COND(c[D_CBC_RC5][testnum]); count++)
        RC5_32_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                           (size_t)lengths[testnum], &rc5_ks, iv, RC5_ENCRYPT);
      d = Time_F(STOP);
    ...
    for (testnum = 0; testnum < size_num && async_init == 0; testnum++) {
      ...
      for (count = 0, run = 1; COND(c[D_CBC_BF][testnum]); count++)
        BF_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                       (size_t)lengths[testnum], &bf_ks, iv, BF_ENCRYPT);
      d = Time_F(STOP);
    ...
    for (testnum = 0; testnum < size_num && async_init == 0; testnum++) {
      ...
      for (count = 0, run = 1; COND(c[D_CBC_CAST][testnum]); count++)
        CAST_cbc_encrypt(loopargs[0].buf, loopargs[0].buf,
                         (size_t)lengths[testnum], &cast_ks, iv, CAST_ENCRYPT);
      d = Time_F(STOP);
  ...
  if (doit[D_EVP]) {
    if (evp_cipher != NULL) {
      ...
      if (multiblock &&
          (EVP_CIPHER_flags(evp_cipher) & EVP_CIPH_FLAG_TLS1_1_MULTIBLOCK)) {
      ...
      for (testnum = 0; testnum < size_num; testnum++) {
        print_message(names[D_EVP], save_count, lengths[testnum], seconds.sym);

        for (k = 0; k < loopargs_len; k++) {
          loopargs[k].ctx = EVP_CIPHER_CTX_new();
          EVP_CipherInit_ex(loopargs[k].ctx, evp_cipher, NULL, NULL, iv,
                            decrypt ? 0 : 1);
          ...
          EVP_CipherInit_ex(loopargs[k].ctx, NULL, NULL, loopargs[k].key, NULL,
                            -1);
  ...
  for (testnum = 0; testnum < RSA_NUM; testnum++) {
    ...
    if (st == 0) {
      ...
      pkey_print_message("private", "rsa", rsa_c[testnum][0], rsa_bits[testnum],
                         seconds.rsa);
    ...
    if (st <= 0) {
      ...
      pkey_print_message("public", "rsa", rsa_c[testnum][1], rsa_bits[testnum],
                         seconds.rsa);
      Time_F(START);
  ...
  for (testnum = 0; testnum < DSA_NUM; testnum++) {
    ...
    if (st == 0) {
      ...
      pkey_print_message("sign", "dsa", dsa_c[testnum][0], dsa_bits[testnum],
                         seconds.dsa);
      Time_F(START);
    ...
    if (st <= 0) {
      ...
      pkey_print_message("verify", "dsa", dsa_c[testnum][1], dsa_bits[testnum],
                         seconds.dsa);
      Time_F(START);
  ...
  for (testnum = 0; testnum < EC_NUM; testnum++) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (!(test_ctx =
                EVP_PKEY_CTX_new(key_B, NULL)) || /* test ctx from skeyB */
    ...
    if (st == 0) {
      ...
      if (st == 0) {
        ...
        pkey_print_message("sign", test_ed_curves[testnum].name,
                           eddsa_c[testnum][0], test_ed_curves[testnum].bits,
                           seconds.eddsa);
        Time_F(START);
      ...
      for (i = 0; i < loopargs_len; i++) {
        st = EVP_DigestVerify(loopargs[i].eddsa_ctx[testnum], loopargs[i].buf2,
                              loopargs[i].sigsize, loopargs[i].buf, 20);
        if (st != 1)
          break;
      }
      if (st != 1) {
        ...
        pkey_print_message("verify", test_ed_curves[testnum].name,
                           eddsa_c[testnum][1], test_ed_curves[testnum].bits,
                           seconds.eddsa);
        Time_F(START);
...
show_res:
  ...
  for (k = 0; k < OSSL_NELEM(ecdsa_doit); k++) {
    ...
    if (mr)
      printf("+F4:%u:%u:%f:%f\n", k, test_curves[k].bits, ecdsa_results[k][0],
             ecdsa_results[k][1]);
    else
  ...
  for (k = 0; k < EC_NUM; k++) {
    ...
    if (mr)
      printf("+F5:%u:%u:%f:%f\n", k, test_curves[k].bits, ecdh_results[k][0],
             1.0 / ecdh_results[k][0]);

    else
      printf("%4u bits ecdh (%s) %8.4fs %8.1f\n", test_curves[k].bits,
             test_curves[k].name, 1.0 / ecdh_results[k][0], ecdh_results[k][0]);
  }
  ...
  for (k = 0; k < OSSL_NELEM(eddsa_doit); k++) {
    ...
    if (mr)
      printf("+F6:%u:%u:%s:%f:%f\n", k, test_ed_curves[k].bits,
             test_ed_curves[k].name, eddsa_results[k][0], eddsa_results[k][1]);
    else
