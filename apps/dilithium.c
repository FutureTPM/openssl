...
typedef enum OPTION_choice {
  ...
  OPT_PASSOUT,
  OPT_PASSIN,
  OPT_DILITHIUMPUBKEY_IN,
  OPT_DILITHIUMPUBKEY_OUT,
  OPT_NOOUT,
  OPT_TEXT,
  OPT_CHECK,
  OPT_CIPHER
...
const OPTIONS dilithium_options[] = {
    ...
    {"DilithiumPublicKey_in", OPT_DILITHIUMPUBKEY_IN, '-',
     "Input is an DilithiumPublicKey"},
    {"DilithiumPublicKey_out", OPT_DILITHIUMPUBKEY_OUT, '-',
     "Output is an DilithiumPublicKey"},
...
private
  ...
  if (outformat == FORMAT_ASN1) {
    ...
    if (pubout || pubin) {
      ...
      assert(private);
      i = PEM_write_bio_DilithiumPrivateKey(out, dilithium, enc, NULL, 0, NULL,
                                            passout);
    }
  } else {
