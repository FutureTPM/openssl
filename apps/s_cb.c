...
int generate_cookie_callback(SSL *ssl, unsigned char *cookie,
  ...
  HMAC(EVP_sha1(), cookie_secret, COOKIE_SECRET_LENGTH, buffer, length, cookie,
       cookie_len);
...
#define TLSA_TAIL_SIZE 12
    ...
    BIO_printf(bio, "DANE TLSA %d %d %d %s%s %s at depth %d\n", usage, selector,
               mtype, (dlen > TLSA_TAIL_SIZE) ? "..." : "", hexdata,
...
static int security_callback_debug(const SSL *s, const SSL_CTX *ctx, int op,
  ...
  case SSL_SECOP_OTHER_CERT: {
    if (cert_md) {
      ...
      BIO_printf(sdb->out, "%s, bits=%d", algname, EVP_PKEY_bits(pkey));
    }
    break;
  }
