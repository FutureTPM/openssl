...
static void sv_usage(void) {
  ...
  fprintf(stderr, " -no_ecdhe     - disable ECDHE\nTODO(openssl-team): "
                  "no_ecdhe was broken by auto ecdh. Make this work again.\n");
#endif
#ifndef OPENSSL_NO_PSK
  ...
  fprintf(stderr, " -alpn_expected <string> - the ALPN protocol that should be "
                  "negotiated\n");
  ...
  fprintf(stderr, " -should_reuse <number>     - The expected state of reusing "
                  "the session\n");
...
int main(int argc, char *argv[]) {
  ...
  while (argc >= 1) {
    if (strcmp(*argv, "-F") == 0) {
      ...
      dhe1024dsa = 1;
#else
      fprintf(stderr, "ignoring -dhe1024dsa, since I'm compiled without DH\n");
#endif
  ...
  if (print_time) {
    ...
    if (number < 50 && !force)
      fprintf(stderr, "Warning: For accurate timings, use more connections "
                      "(e.g. -num 1000)\n");
  }
