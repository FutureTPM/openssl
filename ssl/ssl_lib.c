...
int SSL_is_dtls(const SSL *s) { return SSL_IS_DTLS(s) ? 1 : 0; }
...
SSL_DANE *SSL_get0_dane(SSL *s) { return &s->dane; }
...
long SSL_get_default_timeout(const SSL *s) { return s->method->get_timeout(); }
...
int SSL_get_early_data_status(const SSL *s) { return s->ext.early_data; }
...
int SSL_get_key_update_type(const SSL *s) { return s->key_update; }
...
int SSL_export_keying_material_early(SSL *s, unsigned char *out, size_t olen,
  ...
  return tls13_export_keying_material_early(s, out, olen, label, llen, context,
                                            contextlen);
}
...
int ssl_undefined_const_function(const SSL *s) { return 0; }
...
size_t SSL_get_num_tickets(const SSL *s) { return s->num_tickets; }
...
int SSL_session_reused(SSL *s) { return s->hit; }

int SSL_is_server(const SSL *s) { return s->server; }
...
void SSL_set_security_level(SSL *s, int level) { s->cert->sec_level = level; }
...
void SSL_set0_security_ex_data(SSL *s, void *ex) { s->cert->sec_ex = ex; }

void *SSL_get0_security_ex_data(const SSL *s) { return s->cert->sec_ex; }
 ...
 * control interface.
 */
unsigned long SSL_CTX_get_options(const SSL_CTX *ctx) { return ctx->options; }

unsigned long SSL_get_options(const SSL *s) { return s->options; }
...
int SSL_alloc_buffers(SSL *ssl) { return ssl3_setup_buffers(ssl); }
...
int ssl_log_rsa_client_key_exchange(SSL *ssl,
  ...
  if (encrypted_premaster_len < 8) {
    SSLfatal(ssl, SSL_AD_INTERNAL_ERROR, SSL_F_SSL_LOG_RSA_CLIENT_KEY_EXCHANGE,
             ERR_R_INTERNAL_ERROR);
    return 0;
  }
...
int bytes_to_cipher_list(SSL *s, PACKET *cipher_suites,
  ...
  if (PACKET_remaining(cipher_suites) % n != 0) {
    if (fatal)
      ...
      SSLerr(SSL_F_BYTES_TO_CIPHER_LIST, SSL_R_ERROR_IN_RECEIVED_CIPHER_LIST);
    return 0;
  }
...
uint32_t SSL_get_max_early_data(const SSL *s) { return s->max_early_data; }
...
void SSL_set_post_handshake_auth(SSL *ssl, int val) { ssl->pha_enabled = val; }
