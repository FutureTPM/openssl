...
#endif
#ifndef OPENSSL_NO_DILITHIUM
    {"dilithium_with_sha3_256", TLSEXT_SIGALG_dilithium_sha3_256, NID_sha3_256,
     SSL_MD_SHA3_256_IDX, EVP_PKEY_DILITHIUM, SSL_PKEY_DILITHIUM,
     NID_dilithium_with_SHA3_256, NID_undef},
    {"dilithium_with_sha3_384", TLSEXT_SIGALG_dilithium_sha3_384, NID_sha3_384,
     SSL_MD_SHA3_384_IDX, EVP_PKEY_DILITHIUM, SSL_PKEY_DILITHIUM,
     NID_dilithium_with_SHA3_384, NID_undef},
    {"dilithium_with_sha3_512", TLSEXT_SIGALG_dilithium_sha3_512, NID_sha3_512,
     SSL_MD_SHA3_512_IDX, EVP_PKEY_DILITHIUM, SSL_PKEY_DILITHIUM,
     NID_dilithium_with_SHA3_512, NID_undef},
    {"dilithium_with_sha256", TLSEXT_SIGALG_dilithium_sha256, NID_sha256,
     SSL_MD_SHA256_IDX, EVP_PKEY_DILITHIUM, SSL_PKEY_DILITHIUM,
     NID_dilithium_with_SHA256, NID_undef},
    {"dilithium_with_sha384", TLSEXT_SIGALG_dilithium_sha384, NID_sha384,
     SSL_MD_SHA384_IDX, EVP_PKEY_DILITHIUM, SSL_PKEY_DILITHIUM,
     NID_dilithium_with_SHA384, NID_undef},
    {"dilithium_with_sha512", TLSEXT_SIGALG_dilithium_sha512, NID_sha512,
     SSL_MD_SHA512_IDX, EVP_PKEY_DILITHIUM, SSL_PKEY_DILITHIUM,
     NID_dilithium_with_SHA512, NID_undef},
    {"dilithium_with_sha1", TLSEXT_SIGALG_dilithium_sha1, NID_sha1,
     SSL_MD_SHA1_IDX, EVP_PKEY_DILITHIUM, SSL_PKEY_DILITHIUM,
...
SSL_TICKET_STATUS tls_decrypt_ticket(SSL *s, const unsigned char *etick,
  ...
  if (tctx->ext.ticket_key_cb) {
    ...
    int rv = tctx->ext.ticket_key_cb(s, nctick, nctick + TLSEXT_KEYNAME_LENGTH,
                                     ctx, hctx, 0);
    if (rv < 0) {
...
int tls12_copy_sigalgs(SSL *s, WPACKET *pkt, const uint16_t *psig,
  ...
  for (i = 0; i < psiglen; i++, psig++) {
     ...
     */
    if (rv == 0 &&
        (!SSL_IS_TLS13(s) || (lu->sig != EVP_PKEY_RSA && lu->hash != NID_sha1 &&
                              lu->hash != NID_sha224)))
      rv = 1;
  }
  if (rv == 0)
...
static int has_usable_cert(SSL *s, const SIGALG_LOOKUP *sig, int idx) {
  ...
  if (s->s3->tmp.peer_cert_sigalgs != NULL) {
    for (i = 0; i < s->s3->tmp.peer_cert_sigalgslen; i++) {
      ...
      if (lu == NULL ||
          !X509_get_signature_info(s->cert->pkeys[idx].x509, &mdnid, &pknid,
                                   NULL, NULL) ||
  ...
  if (SSL_IS_TLS13(s)) {
    ...
    for (i = 0; i < s->cert->shared_sigalgslen; i++) {
      ...
      if (lu->sig == EVP_PKEY_EC) {
        ...
        if (curve == -1) {
          EC_KEY *ec =
              EVP_PKEY_get0_EC_KEY(s->cert->pkeys[SSL_PKEY_ECC].privatekey);
    ...
    if (SSL_USE_SIGALGS(s)) {
      ...
      if (s->s3->tmp.peer_sigalgs != NULL) {
        ...
        if (i == sent_sigslen) {
          if (!fatalerrs)
            return 1;
          SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_F_TLS_CHOOSE_SIGALG,
                   SSL_R_WRONG_SIGNATURE_TYPE);
          return 0;
        }
      }
