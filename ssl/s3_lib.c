...
long ssl3_ctrl(SSL *s, int cmd, long larg, void *parg) {
  ...
  case SSL_CTRL_SET_TMP_DH: {
    ...
    if (!ssl_security(s, SSL_SECOP_TMP_DH, EVP_PKEY_security_bits(pkdh), 0,
                      pkdh)) {
...
long ssl3_ctx_ctrl(SSL_CTX *ctx, int cmd, long larg, void *parg) {
  ...
  case SSL_CTRL_SET_TMP_DH: {
    ...
    if (!ssl_ctx_security(ctx, SSL_SECOP_TMP_DH, EVP_PKEY_security_bits(pkdh),
                          0, pkdh)) {
...
int ssl3_get_req_cert_type(SSL *s, WPACKET *pkt) {
   ...
   */
  if (s->version >= TLS1_VERSION && !(alg_a & SSL_aDILITHIUM) &&
