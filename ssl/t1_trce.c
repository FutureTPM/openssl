...
static const ssl_trace_tbl ssl_ciphers_tbl[] = {
    ...
    {0xCCAF, "TLS_KYBER_RSA_WITH_AES_256_GCM_SHA384"},
    {0xCCB0, "TLS_KYBER_RSA_WITH_AES_128_GCM_SHA256"},
    {0xCCB1, "TLS_KYBER_RSA_WITH_AES_256_CBC_SHA384"},
...
static int ssl_print_extension(BIO *bio, int indent, int server,
  ...
  case TLSEXT_TYPE_early_data:
    ...
    if (extlen != 4)
      return 0;
    max_early_data = (ext[0] << 24) | (ext[1] << 16) | (ext[2] << 8) | ext[3];
    BIO_indent(bio, indent + 2, 80);
...
void SSL_trace(int write_p, int version, int content_type, const void *buf,
  ...
  case SSL3_RT_HEADER: {
    ...
    if (msglen < (size_t)(SSL_IS_DTLS(ssl) ? DTLS1_RT_HEADER_LENGTH
  ...
  case SSL3_RT_HANDSHAKE:
    if (!ssl_print_handshake(bio, ssl, ssl->server ? write_p : !write_p, msg,
                             msglen, 4))
