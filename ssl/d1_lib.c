...
int DTLSv1_listen(SSL *s, BIO_ADDR *client) {
  ...
  do {
    ...
    if (s->msg_callback)
      s->msg_callback(0, 0, SSL3_RT_HEADER, buf, DTLS1_RT_HEADER_LENGTH, s,
                      s->msg_callback_arg);
    ...
    if (s->msg_callback)
      s->msg_callback(0, s->version, SSL3_RT_HANDSHAKE, data,
                      fraglen + DTLS1_HM_HEADER_LENGTH, s, s->msg_callback_arg);
    ...
    if (next == LISTEN_SEND_VERIFY_REQUEST) {
       ...
       */
      version =
          (s->method->version == DTLS_ANY_VERSION) ? DTLS1_VERSION : s->version;
      ...
      if (s->msg_callback)
        s->msg_callback(1, 0, SSL3_RT_HEADER, buf, DTLS1_RT_HEADER_LENGTH, s,
                        s->msg_callback_arg);
  ...
  if (s->d1->mtu < dtls1_min_mtu(s)) {
    if (!(SSL_get_options(s) & SSL_OP_NO_QUERY_MTU)) {
      ...
      if (s->d1->mtu < dtls1_min_mtu(s)) {
        ...
        BIO_ctrl(SSL_get_wbio(s), BIO_CTRL_DGRAM_SET_MTU, (long)s->d1->mtu,
                 NULL);
      }
    } else
      return 0;
...
void DTLS_set_timer_cb(SSL *s, DTLS_timer_cb cb) { s->d1->timer_cb = cb; }
